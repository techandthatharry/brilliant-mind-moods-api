...
class StravaController extends Controller
{
    ...
    public function splits(Request $request, StravaActivity $activity): JsonResponse
    {
        ...
        // ── Step 1: Always try recorded laps first ────────────────────────────
        // This matches exactly what Strava shows in its "Laps" tab.
        // Auto-laps (e.g. every 1 mile) and manually pressed laps both come
        // through here, so interval sessions are split correctly.
        ...
        // ── Step 2: Fall back to auto-splits if no lap data ───────────────────
        // splits_imperial = per-mile boundaries (runs only)
        // splits_metric   = per-km boundaries (runs only) — Flutter converts
        $data   = $resp->json();
